...
| STC89C52RC | 8 KB | UART bootloader (`stcgal`) | `implemented` |
...
Only the AT89S52 line is `verified`.
...
generation has no way in from software, so the board has to be
**power-cycled while the upload is already running**. The console says when.
`stcgal` does the protocol; install it with `python -m pip install stcgal`.
