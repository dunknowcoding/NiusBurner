...
def test_debug_falls_back_to_python_module(monkeypatch, tmp_path: pathlib.Path):
    image = tmp_path / "image.bin"
    image.write_bytes(b"x")
    ...
    cmd = debugger.burn_command(target="target-a", image=image,
                                confirm="target-a", state_policy="replace")
    assert cmd[:3] == [debugger.sys.executable, "-m", "niusburner.prog"]
...
def test_probe_command_matches_confirm(monkeypatch, tmp_path: pathlib.Path):
    ...
    monkeypatch.setenv("NIUSBURNER_BACKEND", str(tool))
    cmd = debugger.probe_command(target="at89s52", confirm="at89s52")
    assert cmd[0] == str(tool)
    assert cmd[1:] == ["probe", "at89s52", "--confirm", "at89s52"]
    with pytest.raises(ValueError, match="exactly match"):
        debugger.probe_command(target="at89s52", confirm="other")
